...
@Component({
  selector: 'app-bstinsert-node',
  templateUrl: './bstinsert-node.component.html',
  styleUrls: ['./bstinsert-node.component.css']
})
export class BSTInsertNodeComponent {
    ...
    // Tree and nodes
    tree = null;
    nodes = [2.5, 1.5, 3.5, 3, 1.4, 2, 1.8, 2.2, 2.7, 3.7, 4.5];
    branches = [];
  ...
  constructor() { 
    ...
    /* Animate function. It updates all the animations */  
  	animate() {
        ...
        if(this.animation && !this.initAnim) {
            ...
            var sphere = this.createSphere(4);
            sphere.position.x = -60;
            sphere.position.y = 20;
            this.scene.add(sphere);

            var tween1 = new TWEEN.Tween(sphere.position)
                .to({x: -10, y: 20}, this.time)
                .delay(this.delay)
                .start();   

            this.delay += this.time + 1000;

            var node = this.tree.retrieve(this.nodes[0]);

            this.changeColour(sphere, this.time, this.delay, 0x7d1799);
            this.changeColour(node.sphere, this.time, this.delay, 0x7d1799); 
            
            this.delay += this.time;	
	       
            this.changeColour(sphere, this.time, this.delay, 0xbc9c1a);
            this.changeColour(node.sphere, this.time, this.delay, 0x3abc1a);

            var tween2 = new TWEEN.Tween(sphere.position)
                .to({x: 20, y: 23})
                .delay(this.delay);
            ...
            node = this.tree.retrieve(this.nodes[2]);
            ...
            var tween3 = new TWEEN.Tween(sphere.position)
                .to({x: 45, y: 15})
	...
	createSphere(radius) {
        ...
        var material = new THREE.MeshPhongMaterial({color: 0xbc9c1a});
    ...
    insertNode(element, time, delay) {

    	var tween = new TWEEN.Tween(element.position)
            	.to({x: -10, y: 20, z: 10}, this.time)
            	.delay(this.delay)
            	.start();
