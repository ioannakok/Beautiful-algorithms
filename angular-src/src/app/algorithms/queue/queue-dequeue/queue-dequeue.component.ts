...
@Component({
  ...
  templateUrl: './queue-dequeue.component.html',
...
export class QueueDequeueComponent implements OnInit, OnInteract {
  ...
  ngOnInit() {
    ...
    this.algorithm = this.algorithmsService.getAlgorithm(8);
  }
	...
	/* Animate function. It updates all the animations */  
  	animate() {
        ...
        if(this.animation && !this.initAnim) {
        	...
        	this.deQueue(this.bars[this.bars.length - 1], this.time, this.delay);
