...
@Component({
  selector: 'app-queue-front',
  templateUrl: './queue-front.component.html',
  styleUrls: ['./queue-front.component.css'],
  providers: [AlgorithmsService]
})
export class QueueFrontComponent implements OnInit, OnInteract {
  ...
  ngOnInit() {
    ...
    this.algorithm = this.algorithmsService.getAlgorithm(9);
  }
	...
	/* Animate function. It updates all the animations */  
  	animate() {
        ...
        if(this.animation && !this.initAnim) {
        	...
        	// Call the front method
        	this.front(this.bars[this.bars.length - 1], this.time, this.delay);
