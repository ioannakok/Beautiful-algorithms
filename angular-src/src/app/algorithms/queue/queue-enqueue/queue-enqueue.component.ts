...
export class QueueEnqueueComponent implements OnInit, OnInteract {
  ...
  ngOnInit() {
   ...
   this.algorithm = this.algorithmsService.getAlgorithm(7);
  }  
  ...
  ngAfterViewInit() {
	...
	/* Animate function. It updates all the animations */  
  	animate() {
        ...
        if(this.animation && !this.initAnim) {
        	...
        	var bar = this.getBox(-50, 50, 0, 0xffff00);
        	this.scene.add(bar);
        	this.bars.push(bar);
    ...
    enqueue(element, x, y, time, delay) {

    	// Element gets into the scene
