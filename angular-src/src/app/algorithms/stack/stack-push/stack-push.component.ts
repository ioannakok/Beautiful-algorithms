...
export class StackPushComponent implements OnInit, OnInteract {
    ...
    this.algorithm = this.algorithmsService.getAlgorithm(4);
  }  
	...
	/* Animate function. It updates all the animations */  
  	animate() {
        ...
        if(this.animation && !this.initAnim) {
        	...
        	this.animateCode(this.python1, this.time, pythonDelay);

        	// Time for Java and C++
        	time = this.time;
        	var delay = 0;

        	this.animateCode(this.java1, time, delay);
        	this.animateCode(this.c1, time, delay);
            this.animateCode(this.pseudo0, time, delay);
        	delay += time;

        	this.animateCode(this.java5, time, delay);
        	this.animateCode(this.c5, time, delay);
            this.animateCode(this.pseudo2, time, delay);
        	delay += time;

        	this.animateCode(this.java6, time, delay);
        	this.animateCode(this.c6, time, delay);
            this.animateCode(this.pseudo3, time, delay);
            this.animateCode(this.pseudo4, time, delay);
        	delay += time;
